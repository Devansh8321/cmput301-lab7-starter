...
public class MainActivityTest {
    ...
    // -------------------------
    // Added ShowActivity tests (same functions you had)
    // -------------------------
    @Test
    public void testNavigateToShowActivity() {
        ...
        onData(is(instanceOf(String.class)))
                .inAdapterView(withId(R.id.city_list)).atPosition(0).perform(click());
        onView(withId(R.id.text_city)).check(matches(isDisplayed()));
    }

    // Check city name consistency
    @Test
    public void testDisplayedCityNameMatchesSelection() {
        onView(withId(R.id.button_add)).perform(click());
        ...
        onView(withId(R.id.text_city)).check(matches(withText("Toronto")));
    }
    ...
    public void testReturnToMainActivity() {
        ...
        onView(withId(R.id.button_confirm)).perform(click());
        onData(is("Montreal")).inAdapterView(withId(R.id.city_list)).perform(click());
        onView(withId(R.id.back_button)).perform(click());
        onView(withId(R.id.city_list)).check(matches(isDisplayed()));
        onView(withId(R.id.text_city)).check(doesNotExist());
    }
}
